...
*(Insert screenshot: intake → decision workspace)*
...
```
Resident text + optional document
            |
   ┌────────┴────────┐
   │  Strands Agent   │   agent.tool.extract_from_text / extract_from_document
   │  (tool-orchestrated,
   │   not free-form)  │
   └────────┬────────┘
            |
   Pydantic-validated structured evidence
   (fields + per-field confidence + source)
            |
   ┌────────┴────────┐
   │  Merge evidence   │   text + document reconciled; source="both" on agreement
   └────────┬────────┘
            |
   ┌────────┴────────┐
   │ Contradiction gate│   >5% income disagreement or exact-match mismatch → STOP
   └────────┬────────┘
            |
   ┌────────┴────────┐
   │ Next-Best-Action  │   required fields missing/low-confidence → follow-up question
   └────────┬────────┘
            |
   ┌────────┴────────┐
   │ Deterministic     │   DC DOEE FY26 income + crisis rules, benefit lookup
   │ eligibility engine│
   └────────┬────────┘
            |
   ┌────────┴────────┐
   │  Triage ranking   │   crisis > urgent-incomplete > contradiction > eligible > ...
   └───────────────────┘
```
...
**Why Groq:** the project began on Amazon Bedrock. Bedrock access was blocked during development, so extraction was migrated to Groq (`openai/gpt-oss-120b`) to keep development moving. This is documented as a development-speed decision, not a permanent architectural stance — see [Known Limitations](#known-limitations--roadmap).
...
├── manual_test.py            # Manual single-case testing during development
└── tests/
...
- **Document support is plain text only.** No OCR or PDF parsing yet. A real intake document would need to be transcribed to text first; this is a scope boundary for the hackathon timeline, not an oversight.
...
- **Confidence is currently a raw per-field float, not yet tiered.** It's load-bearing (used by Next-Best-Action to distrust low-confidence guesses) but has observed run-to-run jitter of roughly ±0.1–0.2 at fixed temperature. Bucketing into high/medium/low tiers is a planned refinement.
- **`stated_deadline_or_urgency` sometimes captures situational deadlines (e.g., a bill's due date) rather than the applicant's own expressed urgency.** Logged, not yet fixed — doesn't affect eligibility decisions, since urgency detection for triage relies primarily on the structured `disconnection_status` field.
...
[Your Builder ID / build story link, if published to builder.aws.com]
